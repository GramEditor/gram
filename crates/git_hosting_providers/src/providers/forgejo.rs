...
mod tests {
    ...
    fn test_build_forgejo_self_hosted_permalink_from_ssh_url() {
        let forgejo = Forgejo::from_remote_url("git@forgejo.some-enterprise.com:GramEditor/gram.git").unwrap();
    ...
    fn test_build_forgejo_self_hosted_permalink_from_https_url() {
        let forgejo = Forgejo::from_remote_url("https://forgejo-instance.big-co.com/GramEditor/gram.git").unwrap();
        let permalink = forgejo.build_permalink(
            ParsedGitRemote {
                owner: "GramEditor".into(),
                repo: "gram".into(),
